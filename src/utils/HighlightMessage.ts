...
const highlightCodeBlock = (codeBlock: string) => {
  ...
  const code = codeBlock.replace(/<code[^>]*>/, '').replace(/<\/code>/, '');
...
const separateInputChunks = async (text: string): Promise<Chunk[]> => {
  ...
  for (const codeBlock of wrappedCodeBlocks) {
    ...
    if (original)
      chunks.push({ raw: original, output: await parseTextChunk(original), type: 'text' });
    chunks.push({ raw: codeBlock, output: await parseCodeChunk(codeBlock), type: 'code' });
    input = rest;
  }

  if (input.length > 0)
    chunks.push({ raw: input, output: await parseTextChunk(input), type: 'text' });
